...
public class ConvexHull 
{
	...
	public ConvexHull(int n) throws IllegalArgumentException 
	{
		...
		for(int i = 0; i < n; i++){
			points[i] = new Point(50 - rand.nextInt(101), 50 - rand.nextInt(101));
		}
	}
	...
	public void lowestPoint()
	{
		...
		for(int i = 1; i < points.length; i++){
			if(points[i].getY() < small.getY()) small = points[i]; //if we find a lower point, set it
			
			else if(points[i].getY() == small.getY()){ //if we find an equal point, find a smaller x
				if(points[i].getX() < small.getX()) small = points[i];
			}
